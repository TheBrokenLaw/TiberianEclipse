...
public class WorldGenArboreaPod extends WorldGenerator {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        {
            BlockPos blockpos4 = position.add(rand.nextInt(2) - rand.nextInt(2), rand.nextInt(4) - rand.nextInt(4), rand.nextInt(2) - rand.nextInt(2));

            if (worldIn.isAirBlock(blockpos4) && ModBlocks.arboreaPod.canBlockStay(worldIn, blockpos4, this.arboreaPodState))
            {
                this.setBlockAndAge(rand, worldIn, blockpos4, this.arboreaPodState.withProperty(ModBlocks.arboreaPod.AGE, this.randomAge(rand)));
                this.setBlock(rand, worldIn, blockpos4.down(), this.fieldGrassState);
            }
